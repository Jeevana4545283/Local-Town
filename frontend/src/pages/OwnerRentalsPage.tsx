...
export function OwnerRentalsPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.status === 401) {
          ...
          alert(`Failed to ${editingId ? 'update' : 'add'} rental`)
        }
        setIsPublishing(false)
      }
    } catch (err) {
  ...
  const handleEdit = (rental: any) => {
    ...
    setPincode(rental.pincode.toString())
    setNearbyAreas(rental.nearbyAreas.join(', '))
    setRentPrice(rental.rentPrice.toString())
    setPhoneNumber(rental.phoneNumber)
    setDescription(rental.description)
    setImageUrls(rental.imageUrls || [])
    setStatus(rental.status)
    setPosition([rental.latitude, rental.longitude])
    setShowModal(true)
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">My Rentals</h1>
          <p className="mt-2 text-zinc-500 dark:text-zinc-400">Manage your existing rental properties or add new ones.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {rentals.length === 0 && (
          <div className="col-span-full py-20 text-center text-zinc-500 border-2 border-dashed border-zinc-200 dark:border-white/10 rounded-2xl">
            ...
            <p className="mt-2">You haven't posted any rental properties yet.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white dark:bg-zinc-900 w-full max-w-4xl rounded-3xl shadow-2xl my-8">
            <div className="p-6 border-b border-zinc-200 dark:border-white/10 flex justify-between items-center sticky top-0 bg-white dark:bg-zinc-900 z-10 rounded-t-3xl">
              <h2 className="text-2xl font-bold dark:text-white">{editingId ? 'Edit Rental' : 'Add New Rental'}</h2>
              <button onClick={() => setShowModal(false)} className="text-zinc-500 hover:text-zinc-800 dark:hover:text-white">✕</button>
            </div>
            
            <form onSubmit={handleAddRental} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-bold mb-1 dark:text-zinc-300">Rent Price (₹)</label>
                    <input required type="number" value={rentPrice} onChange={e=>setRentPrice(e.target.value)} className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent p-3 dark:text-white" />
                  </div>
                </div>
                ...
                <div className="pt-6">
                  ...
                  >
                    {publishSuccess ? (
                      ...
                      editingId ? 'Save Changes' : 'Publish Rental'
                    )}
                  </button>
                </div>
              </div>
      ...
      {showToast && (
        <div className="fixed bottom-6 right-6 z-[200] flex items-center gap-3 rounded-2xl bg-zinc-900 dark:bg-white px-6 py-4 shadow-2xl border border-zinc-800 dark:border-zinc-200 animate-in slide-in-from-bottom-5">
          ...
          <div>
            <div className="text-sm font-bold text-white dark:text-zinc-900">Rental {editingId ? 'Updated' : 'Published'} Successfully!</div>
