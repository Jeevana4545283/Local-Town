...
export function OwnerWorkersBookingsPage() {
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      ...
      if (res.ok) {
        fetchWorkerBookings() // Refresh list
      }
    } catch (err) {
      console.error(err)
    }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">WorkerBooking Requests</h1>
        <p className="mt-2 text-zinc-500 dark:text-zinc-400">Manage all visit requests and workerBookings for your properties.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black dark:text-white">{workerBookings.filter(b => b.status === 'Pending').length}</div>
          </div>
        </Card>
        <Card className="p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-black dark:text-white">{workerBookings.filter(b => b.status === 'Accepted').length}</div>
          </div>
        </Card>
        <Card className="p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 flex items-center gap-4">
          ...
          <div>
            <div className="text-sm font-bold text-zinc-500">Total WorkerBookings</div>
            <div className="text-2xl font-black dark:text-white">{workerBookings.length}</div>
          </div>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10">
        {loading ? (
          <div className="p-12 text-center text-zinc-500">Loading workerBookings...</div>
        ) : workerBookings.length === 0 ? (
          <div className="p-12 text-center text-zinc-500 flex flex-col items-center">
            <CalendarDays size={48} className="opacity-20 mb-4" />
            <div className="text-lg font-bold">No workerBookings yet</div>
                  ...
                  <tr key={b.id} className="hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <img src={b.worker?.imageUrls?.[0] || 'https://via.placeholder.com/100'} className="size-12 rounded-lg object-cover" />
                        <span className="font-bold text-zinc-900 dark:text-white">{b.worker?.title || 'Unknown Property'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
