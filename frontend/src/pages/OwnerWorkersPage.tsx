...
export function OwnerWorkersPage() {
  ...
  const [hourlyRate, setWorkerPrice] = useState('')
  ...
  const handleAddWorker = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      nearbyAreas: nearbyAreas ? nearbyAreas.split(',').map(s => s.trim()) : undefined,
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.status === 401) {
          ...
          alert(`Failed to ${editingId ? 'update' : 'add'} worker`)
        }
        setIsPublishing(false)
      }
    } catch (err) {
  ...
  const handleEdit = (worker: any) => {
    ...
    setPincode(worker.pincode.toString())
    setNearbyAreas(worker.nearbyAreas ? worker.nearbyAreas.join(', ') : '')
    setWorkerPrice(worker.hourlyRate.toString())
    setPhone(worker.phone)
    setDescription(worker.description || '')
    setImageUrls(worker.imageUrls || [])
    setStatus(worker.status)
    setPosition(worker.latitude && worker.longitude ? [worker.latitude, worker.longitude] : null)
    setShowModal(true)
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">My Workers</h1>
          <p className="mt-2 text-zinc-500 dark:text-zinc-400">Manage your existing worker properties or add new ones.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {workers.map((r) => (
          <Card key={r.id} className="overflow-hidden flex flex-col hover:shadow-xl transition-all hover:scale-[1.02]">
            ...
            <div className="p-5 flex flex-col flex-1">
              <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-1">{r.name} - {r.profession}</h3>
        ...
        {workers.length === 0 && (
          <div className="col-span-full py-20 text-center text-zinc-500 border-2 border-dashed border-zinc-200 dark:border-white/10 rounded-2xl">
            ...
            <p className="mt-2">You haven't posted any worker properties yet.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white dark:bg-zinc-900 w-full max-w-4xl rounded-3xl shadow-2xl my-8">
            <div className="p-6 border-b border-zinc-200 dark:border-white/10 flex justify-between items-center sticky top-0 bg-white dark:bg-zinc-900 z-10 rounded-t-3xl">
              <h2 className="text-2xl font-bold dark:text-white">{editingId ? 'Edit Worker' : 'Add New Worker'}</h2>
              <button onClick={() => setShowModal(false)} className="text-zinc-500 hover:text-zinc-800 dark:hover:text-white">✕</button>
            </div>
            
            <form onSubmit={handleAddWorker} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              
              {/* Left Column */}
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold mb-1 dark:text-zinc-300">Worker Name *</label>
                  <input required value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Rahul Kumar" className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent p-3 dark:text-white" />
                </div>

                <div>
                  <label className="block text-sm font-bold mb-1 dark:text-zinc-300">Profession / Work Type *</label>
                  <input required value={profession} onChange={e=>setProfession(e.target.value)} placeholder="e.g. Plumber, Electrician" className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent p-3 dark:text-white" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-bold mb-1 dark:text-zinc-300">Hourly Rate (₹) *</label>
                    <input required type="number" value={hourlyRate} onChange={e=>setWorkerPrice(e.target.value)} className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent p-3 dark:text-white" />
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-bold mb-1 dark:text-zinc-300">Phone Number *</label>
                  <input required type="number" value={phone} onChange={e=>setPhone(e.target.value)} className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent p-3 dark:text-white" />
                </div>

                <div>
                  <label className="block text-sm font-bold mb-1 dark:text-zinc-300">Nearby Areas (Optional)</label>
                  <input value={nearbyAreas} onChange={e=>setNearbyAreas(e.target.value)} placeholder="Madhapur, Gachibowli..." className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent p-3 dark:text-white" />
                </div>

                <div>
                  <label className="block text-sm font-bold mb-1 dark:text-zinc-300">Description / Experience (Optional)</label>
                  <textarea value={description} onChange={e=>setDescription(e.target.value)} rows={3} placeholder="10 years experience..." className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-transparent p-3 dark:text-white" />
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold mb-2 dark:text-zinc-300">Worker Photo (Optional)</label>
                ...
                <div>
                  <div className="flex justify-between items-end mb-1">
                    <label className="block text-sm font-bold dark:text-zinc-300">Exact Live Location (Optional)</label>
                ...
                <div className="pt-6">
                  ...
                  >
                    {publishSuccess ? (
                      ...
                      editingId ? 'Save Changes' : 'Publish Worker'
                    )}
                  </button>
                </div>
              </div>
      ...
      {showToast && (
        <div className="fixed bottom-6 right-6 z-[200] flex items-center gap-3 rounded-2xl bg-zinc-900 dark:bg-white px-6 py-4 shadow-2xl border border-zinc-800 dark:border-zinc-200 animate-in slide-in-from-bottom-5">
          ...
          <div>
            <div className="text-sm font-bold text-white dark:text-zinc-900">Worker {editingId ? 'Updated' : 'Published'} Successfully!</div>
