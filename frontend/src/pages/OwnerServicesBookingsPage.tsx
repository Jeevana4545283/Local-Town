...
export function OwnerServicesBookingsPage() {
  ...
  const fetchServiceBookings = async () => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_BASE || 'http://localhost:4000'}/api/services-bookings/owner`, {
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      const token = localStorage.getItem('lt_token') || ''
      const res = await fetch(`${import.meta.env.VITE_API_BASE || 'http://localhost:4000'}/api/services-bookings/${id}/status`, {
        method: 'PUT',
        headers: { 
